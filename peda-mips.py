...
def getreg(c,s,e):
    return [c + str(i) for i in range(s,e+1)]

REGISTERS = {
    32: ["zero", "at"] + getreg('v',0,1) + getreg('a',0,3) + getreg('t',0,7) + getreg('s',0,7) + getreg('t',8,9) + getreg('k',0,1) + ["gp", "sp", "fp", "ra"],
}
...
class PEDA(object):
    ...
    def set_breakpoint(self, location, temp=None, hard=None):
        ...
        if to_int(location) is not None:
            return self.execute("%s *0x%x" % (cmd, to_int(location)))
        else:
    ...
    @memoized
    def assemble(self, asmcode, arch=None):
        """
    ...
    def get_syscall_args(self, num=None):
        regs = self.getregs()
        if regs is None:
            return None
        r7 = to_int(num)
        ...
        if params_num != 0:
            # deal with mem
            sp = self.getreg("sp")
            mem = self.dumpmem(sp, sp + 4 * params_num)
...
peda.execute("set prompt \001%s\002" % red("\002peda-mips > \001"))  # custom prompt
